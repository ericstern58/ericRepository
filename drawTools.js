...
function getCurvePoints(ptsa, tension, isClosed, numOfSegments) {
	...
	*/
	...
	_pts.unshift(ptsa[0]);					/// copy 1. point and insert at beginning
	_pts.push(ptsa[pl - 1]);	/// copy last point and append
...
/**
 *	Extension to Canvas.context
 *
 *	USAGE:
 *
 *		context.drawCurve(points, tension, isClosed, numberOfSegments, showPoints)
 *
 *		context.drawCurve(array)
 *		context.drawCurve(array, float)
 *		context.drawCurve(array, float, boolean)
 *		context.drawCurve(array, float, boolean, integer)
 *		context.drawCurve(array, float, boolean, integer, boolean)
 *
 *		points				= array of float or integers arranged as x1,y1,x2,y1,...,xn,yn. Minimum 2 points.
 *		tension				= 0-1, 0 = no smoothing, 0.5 = smooth (default), 1 = very smoothed
 *		isClosed			= true = draw a closed curve, false = open ended curve (default)
 ...
 *		NOTE: array must contain a minimum set of two points.
 *		Known bugs: closed curve draws last point wrong.
 *		Known issues: as this is currently a wrapper a tiny performance hit is to be expected (not noticable
